//
//  mcbBase64.cpp
//  crypto
//
//  Created by Radif Sharafullin on 1/23/14.
//
//

#include "mcbBase64.h"
#include <iostream>
namespace mcb{namespace PlatformSupport{namespace crypto{
    static const std::string base64_chars("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
                                          "abcdefghijklmnopqrstuvwxyz"
                                          "0123456789+/");
    
    static inline bool isBase64(byte_t c) {return (isalnum(c) || (c == '+') || (c == '/'));}
    
    std::string base64Encode(byte_t const* buf, unsigned int bufLen) {
        std::string retVal;
        int i(0);
        int j(0);
        byte_t char_array_3[3];
        byte_t char_array_4[4];
        
        while (bufLen--){
            char_array_3[i++] = *(buf++);
            if (i == 3) {
                char_array_4[0] = (char_array_3[0] & 0xfc) >> 2;
                char_array_4[1] = ((char_array_3[0] & 0x03) << 4) + ((char_array_3[1] & 0xf0) >> 4);
                char_array_4[2] = ((char_array_3[1] & 0x0f) << 2) + ((char_array_3[2] & 0xc0) >> 6);
                char_array_4[3] = char_array_3[2] & 0x3f;
                
                for(i=0; (i<4) ; ++i)
                    retVal+=base64_chars[char_array_4[i]];
                i=0;
            }
        }
        
        if (i){
            for(j=i; j<3; ++j)
                char_array_3[j] = '\0';
            
            char_array_4[0] = (char_array_3[0] & 0xfc) >> 2;
            char_array_4[1] = ((char_array_3[0] & 0x03) << 4) + ((char_array_3[1] & 0xf0) >> 4);
            char_array_4[2] = ((char_array_3[1] & 0x0f) << 2) + ((char_array_3[2] & 0xc0) >> 6);
            char_array_4[3] = char_array_3[2] & 0x3f;
            
            for (j = 0; (j < i + 1); j++)
                retVal += base64_chars[char_array_4[j]];
            
            while((i++ < 3))
                retVal+='=';
        }
        
        return retVal;
    }
    
    data_t base64Decode(const std::string & encodedString) {
        int in_len = encodedString.size();
        int i(0);
        int j(0);
        int in_(0);
        byte_t char_array_4[4], char_array_3[3];
        data_t retVal;
        
        while (in_len-- && ( encodedString[in_] != '=') && isBase64(encodedString[in_])) {
            char_array_4[i++] = encodedString[in_]; ++in_;
            if (i==4) {
                for (i=0; i<4; ++i)
                    char_array_4[i] = base64_chars.find(char_array_4[i]);
                
                char_array_3[0] = (char_array_4[0] << 2) + ((char_array_4[1] & 0x30) >> 4);
                char_array_3[1] = ((char_array_4[1] & 0xf) << 4) + ((char_array_4[2] & 0x3c) >> 2);
                char_array_3[2] = ((char_array_4[2] & 0x3) << 6) + char_array_4[3];
                
                for (i = 0; (i < 3); ++i)
                    retVal.emplace_back(char_array_3[i]);
                i = 0;
            }
        }
        
        if (i) {
            for (j=i; j <4; ++j)
                char_array_4[j] = 0;
            
            for (j=0; j <4; ++j)
                char_array_4[j] = base64_chars.find(char_array_4[j]);
            
            char_array_3[0] = (char_array_4[0] << 2) + ((char_array_4[1] & 0x30) >> 4);
            char_array_3[1] = ((char_array_4[1] & 0xf) << 4) + ((char_array_4[2] & 0x3c) >> 2);
            char_array_3[2] = ((char_array_4[2] & 0x3) << 6) + char_array_4[3];
            
            for (j=0; (j<i-1); ++j) retVal.emplace_back(char_array_3[j]);
        }
        
        return retVal;
    }
}}}